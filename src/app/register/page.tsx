  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/auth/register', {
        ...
        body: JSON.stringify({ hotelName, name, email, password }),
      })
    ...
    <div className="min-h-screen nova-bg-animated flex items-center justify-center px-4 relative">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="glass-card p-8">
          <h2 className="text-xl font-bold text-white mb-6">{t('auth.register')}</h2>
          {error && (
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">{t('auth.hotelName')}</label>
              <input type="text" value={hotelName} onChange={(e) => setHotelName(e.target.value)} required className="nova-input" placeholder="Hotel Bellevue" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">{t('auth.yourName')}</label>
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} required className="nova-input" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">{t('auth.email')}</label>
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required autoComplete="email" className="nova-input" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">{t('auth.password')}</label>
              <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required autoComplete="new-password" className="nova-input" />
            </div>
            <button type="submit" disabled={loading} className="w-full nova-btn-primary text-base">
              {loading ? (
